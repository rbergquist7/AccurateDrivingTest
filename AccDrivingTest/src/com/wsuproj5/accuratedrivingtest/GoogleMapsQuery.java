...
public class GoogleMapsQuery {
	...
	public class RoutePatch extends AsyncTask<URL, Integer, String> {
		...
		private String newTerminus;
		
		public RoutePatch(String newOrigin, String newTerminus) {
			this.newOrigin = newOrigin;
			this.newTerminus = newTerminus;
		}
	...
	*/
	/*public class RetrieveRoute extends AsyncTask<URL, Integer, String> {
		...
		protected String doInBackground(URL... params) {
			...
			protected void onPostExecute(String result) {
				//clearMarkers();
				addRoute.createRouteMap.addMarkersToMap(latLngs);
				addRoute.currentWaypoint = destination;
				addRoute.previousWaypoint = origin;
				if (currentRoutes.size() > 1) {
					RetrieveRoute retrieveRoute = new RetrieveRoute(currentRoutes);
		    		retrieveRoute.execute((URL) null);
				} else {
					addRoute.previousWaypoint = addRoute.currentWaypoint;
				}
			}
